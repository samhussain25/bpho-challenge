...
####### GRAPH #######
        ...
        p = Particle(0, app.speed, app.theta, app.a)
        p.sim()
        ax = fig.add_subplot()
        
        def apogee():
            apogeex=app.speed**2/app.g * np.sin(np.deg2rad(app.theta))*np.cos(np.deg2rad(app.theta))
            apogeey = 0 + app.speed**2/(2*app.g) * np.sin(np.deg2rad(app.theta))**2
        ...
        def high_low():
            ...
            b = -self.X
            c = self.Y - 0 + ( (app.g*self.X**2) / (2*app.speed**2) )
            ...
            print(theta1, theta2)
            l = Particle(h=0, speed=app.speed, theta=np.rad2deg(theta1), a=app.a)
            l.sim()
            h = Particle(h=0, speed=app.speed, theta=np.rad2deg(theta2), a=app.a)
            h.sim()
        ...
        def new_line():
            ...
            p = Particle(h=0, speed=app.speed, theta=app.theta, a=app.a)
            p.sim()
            ...
            for i in range(2):
                top[i]=round(top[i], 2)    
            top = tuple(top)
            ax.set_title(str('g='+str(app.g)+'m/s^-2, h='+str(0)+'m, u='+str(app.speed)+
                             'm/s, θ='+str(app.theta)+'°, apogee='+str(top)+', target=('+
